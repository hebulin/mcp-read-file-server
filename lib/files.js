    ...
    return locked([source, destination], async () => {
      ...
      async function visit(src, dst) {
        ...
        if (st.isDirectory()) {
          ...
          const result = await copyOne(src, dst, settings);
      ...
      try {
        ...
        error.changed = completed.some(x => x.changed) || createdDirectories.length > 0;
        error.partial = completed.map(x => x.destination).slice(0, 100);
