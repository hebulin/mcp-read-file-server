...
/** 按行分页，到达请求结束行即停；未探测EOF时nextLine为待确认的续读位置。 */
async function readLines(file, start, end, options = {}) {
  ...
  for await (const line of textLines(file, options)) {
    ...
    if (chars + line.length + 16 > READ_LIMIT) {
      if (!lines.length) throw fault('LINE_TOO_LONG', '该行加上行号后超过单页预算，请使用字符分页');
      nextLine = lineNo;
      break;
    }
    lines.push({ line: lineNo, text: line });
    chars += line.length + 16;
    // 不请求迭代器生成下一整行，否则范围外的长行也会让当前页失败。
    if (lineNo === end) { nextLine = lineNo + 1; break; }
  }
